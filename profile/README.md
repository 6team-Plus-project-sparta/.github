...
| **팀명**        | **HOT6**                                                    |
| **프로젝트명**     | **티켓을 JAVA라**                                               |
| **플랫폼명**      | **TicketFlow** — 공연·스포츠 티켓팅 플랫폼                             |
| **버전**        | v1.0.0 (MVP)                                                |
...
| 역할                | 이름      | 주요 담당 기능                                                               |
| ----------------- | ------- | ---------------------------------------------------------------------- |
| **E / Infra**     | **정태규** | Docker 기반 개발·실행 환경 구성, AWS 인프라 구성, GitHub Actions CI/CD, DB 인덱스 최적화 지원 |
| **C / Booking**   | **이지민** | 좌석 상태 조회, 좌석 선택, 좌석 Hold, 티켓 예매/취소, Mock PG 결제 연동, 예매 동시성 제어           |
| **B / Event**     | **선경안** | 공연·스포츠 이벤트 CRUD, 이벤트 상세/검색, 구역·좌석 관리, 검색 캐싱, 인기 검색어                    |
| **D / Promotion** | **강태훈** | 선착순 쿠폰 발급/적용, 쿠폰 중복 사용 방지, 쿠폰 재고 동시성 제어, 실시간 CS 채팅, FAQ 챗봇             |
| **A / User**      | **임하은** | 회원가입/로그인, JWT 인증/인가, 내 정보 관리, 찜, 최근 본 내역, 내 예매 내역/상세 조회, 회원 등급 관리      |
...
* E → B: 인프라 안정화 후 인덱스 최적화 지원
* C → D: 예매 플로우 내 쿠폰 적용 시 락 순서 및 확정 타이밍 협의
